defmodule Jamie.Content.PostRevisionsTest do
  ...
  defp make_post(opts \\ []) do
    {:ok, post} = Content.create_post(ContentFixtures.post_attrs(opts))
    post
  end
  ...
  describe "save_revision/3" do
    test "first save is a snapshot" do
      ...
      assert {:ok, %PostRevision{is_snapshot: true, snapshot: snap, diff: nil}} =
               Content.save_revision(nil, post.id, "first content")
    ...
    test "identical content is a no-op" do
      post = make_post()
      {:ok, _} = Content.save_revision(nil, post.id, "same")

      assert :ok = Content.save_revision(nil, post.id, "same")
    ...
    test "wholesale rewrite snapshots instead of diffing" do
      post = make_post()
      {:ok, _} = Content.save_revision(nil, post.id, "short")

      assert {:ok, %PostRevision{is_snapshot: true}} =
               Content.save_revision(nil, post.id, String.duplicate("x", 200))
    end
    ...
    test "every 50th revision is forced to be a snapshot" do
      ...
      Enum.each(2..49, fn n ->
        {:ok, _} = Content.save_revision(nil, post.id, base <> "edit #{n}\n")
      end)

      # the 50th — forced snapshot
      assert {:ok, %PostRevision{is_snapshot: true, revision_number: 50}} =
               Content.save_revision(nil, post.id, base <> "edit 50\n")
    end

    test "revision_number is assigned by the trigger, monotonically per post" do
      post = make_post()
      {:ok, r1} = Content.save_revision(nil, post.id, "a")
      {:ok, r2} = Content.save_revision(nil, post.id, "b")
      {:ok, r3} = Content.save_revision(nil, post.id, "c")
  ...
  describe "reconstruct_content/2" do
    test "returns snapshot content directly" do
      post = make_post()
      {:ok, rev} = Content.save_revision(nil, post.id, "snapshotted")
      assert Content.reconstruct_content(nil, rev.id) == "snapshotted"
    end
    ...
    test "walks the diff chain back to a snapshot" do
      ...
      {:ok, _} = Content.save_revision(nil, post.id, v2)
      {:ok, _} = Content.save_revision(nil, post.id, v3)
      {:ok, rev4} = Content.save_revision(nil, post.id, v4)

      # confirm we actually have at least one diff in the chain
      assert Enum.any?(revisions(post.id), &(&1.is_snapshot == false))

      assert Content.reconstruct_content(nil, rev4.id) == v4
    end
  end
  ...
  describe "list_revisions/2" do
    test "orders by saved_at descending" do
      post = make_post()
      {:ok, r1} = Content.save_revision(nil, post.id, "one")
      {:ok, r2} = Content.save_revision(nil, post.id, "two")
      {:ok, r3} = Content.save_revision(nil, post.id, "three")

      ids = Content.list_revisions(nil, post.id) |> Enum.map(& &1.id)
  ...
  describe "update_post/3 optimistic locking" do
    test "writes succeed when updated_at matches" do
      post = make_post()
      assert {:ok, updated} = Content.update_post(post, %{markdown: "new body"}, post.updated_at)
      assert updated.markdown == "new body"
    end

    test "returns :conflict when updated_at is stale" do
      post = make_post()
      {:ok, _winner} = Content.update_post(post, %{markdown: "winner"}, post.updated_at)

      assert {:error, :conflict} =
               Content.update_post(post, %{markdown: "loser"}, post.updated_at)

      assert Repo.get!(Jamie.Content.Post, post.id).markdown == "winner"
    end
    ...
    test "a successful update inserts a revision in the same transaction" do
      ...
      {:ok, _} = Content.update_post(post, %{markdown: "next body"}, post.updated_at)
    ...
    test "a conflicted update does not insert a revision" do
      post = make_post()
      {:ok, _} = Content.update_post(post, %{markdown: "winner"}, post.updated_at)
      before_count = length(revisions(post.id))

      {:error, :conflict} = Content.update_post(post, %{markdown: "loser"}, post.updated_at)
